/**
 * Landing — fullscreen scroll-snap using SnapContainer (Swiper.js vertical).
 *
 * 7 slides:
 *   0 — Hero
 *   1 — Como funciona (Marquee + 3 passos)
 *   2 — Pontuação (As 3 Formas de Dominar)
 *   3 — Rankings
 *   4 — Grupos
 *   5 — Recompensas (JBL)
 *   6 — CTA Final + Rodapé
 *
 * Section heights: each slide is 100% of the Swiper container (set by AppShell:
 * h-[100dvh] flex-col → header ≈52px → Swiper fills remaining flex-1).
 * On sections with dense content, overflow-y-auto is enabled via SwiperSlide.
 */
...
/** Full-height slide wrapper: centers content, adds bottom padding so the
 *  StickyPrizeBar (56 px) on mobile doesn't cover the last element. */
const Slide = ({
  children,
  className = "",
  center = true,
  ...
  <div
    id={id}
    className={cn(
      "w-full h-full px-4 pb-16 sm:pb-6",
...
// ─── Scroll hint arrow (slide 0) ──────────────────────────────────────────────

const ScrollHint = ({ visible }: { visible: boolean }) => (
  <motion.div
    animate={{ opacity: visible ? 1 : 0, y: visible ? 0 : 8 }}
    transition={{ duration: 0.4 }}
    className="absolute bottom-6 left-1/2 -translate-x-1/2 flex flex-col items-center gap-1 text-muted-foreground pointer-events-none"
    aria-hidden="true"
  >
...
const Landing = () => {
  ...
  const slideHero = (
    <div
      id="hero"
      className="relative w-full h-full flex flex-col items-center justify-center px-4 overflow-hidden"
    ...
    >
      ...
      <div className="relative z-10 max-w-lg mx-auto text-center space-y-7">
        ...
        <motion.div
          ...
          className="space-y-1"
        >
          <div className="text-sm font-bold uppercase tracking-widest text-muted-foreground">Prêmio Total</div>
  ...
  const slideComo = (
    <Slide id="como-funciona" className="py-10 sm:py-14" center={false}>
      <div className="max-w-lg mx-auto w-full">
        {/* Marquee strip */}
        <div className="overflow-hidden border-y border-border/40 bg-black/40 py-1 mb-8 -mx-4" aria-hidden="true">
        ...
        <div className="space-y-4">
          {[
            { step: "01", icon: Target, title: "Dê seu Palpite", desc: "Escolha os placares dos 104 jogos da Copa do Mundo. Quanto mais preciso, mais pontos você acumula.", delay: 50 },
            { step: "02", icon: Star, title: "Acumule Pontos", desc: "Cada acerto te lança para o topo do ranking — local, estadual e nacional. Multiplicadores de fase turbinando.", delay: 150 },
            { step: "03", icon: Trophy, title: "Suba e Ganhe", desc: "Seja o premiado do jogo, da rodada ou do dia. Pix, prêmios físicos e a grande bolada da final.", delay: 250 },
          ].map(({ step, icon: Icon, title, desc, delay }) => (
            <div key={step} data-aos="fade-up" data-aos-delay={delay}>
  ...
  // ── Slide 2: Pontuação ─────────────────────────────────────────────────────
  const slidePontuacao = (
    <Slide id="pontuacao" className="py-10 sm:py-14" center={false}>
      <div className="max-w-lg mx-auto w-full">
        <div className="text-center mb-8" data-aos="fade-up">
          <div
            className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-bold mb-4"
          ...
          {[
            {
              ...
              highlight: "100 pts base por gabarito perfeito", border: "border-green-400/20", glow: "from-green-500/5", delay: 50,
            },
            {
              ...
              highlight: "Bônus de até 10x na final", border: "border-blue-400/20", glow: "from-blue-500/5", delay: 150,
            },
            {
              ...
              highlight: "+200 pts por indicação confirmada", border: "border-primary/20", glow: "from-primary/5", delay: 250,
            },
          ].map(({ emoji, title, tag, tagColor, tagBg, desc, highlight, border, glow, delay }) => (
            <div key={title} data-aos="fade-up" data-aos-delay={delay}>
              <div className={cn("rounded-2xl border p-5 bg-gradient-to-br to-transparent", border, glow)}>
      ...
      >
        <div className="max-w-lg mx-auto w-full">
          ...
          <div className="grid grid-cols-1 gap-3">
            {[
              { icon: MapPin, emoji: "🏙️", title: "Ranking da sua Cidade", example: "Seja o #1 de Londrina", desc: "Mostre para todo mundo da sua cidade que você entende de futebol mais do que ninguém.", color: "text-emerald-400", border: "border-emerald-400/20", bg: "from-emerald-500/5", delay: 50 },
              { icon: Map, emoji: "🗺️", title: "Ranking do seu Estado", example: "Domine o Paraná", desc: "Seu estado inteiro vai saber quem é o maior especialista em palpites desta Copa.", color: "text-blue-400", border: "border-blue-400/20", bg: "from-blue-500/5", delay: 150 },
              { icon: Globe, emoji: "🇧🇷", title: "Ranking Geral", example: "Dispute R$ 5 Milhões", desc: "A arena máxima. Você contra o Brasil inteiro. O topo paga R$ 5 milhões em prêmios.", color: "text-primary", border: "border-primary/25", bg: "from-primary/8", delay: 250 },
  ...
  // ── Slide 4: Grupos ────────────────────────────────────────────────────────
  const slideGrupos = (
    <Slide id="grupos" className="py-10 sm:py-14" center={false}>
      <div className="max-w-lg mx-auto w-full" data-aos="fade-up">
          ...
          <div className="p-6 space-y-5">
            <div>
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                Crie grupos exclusivos para competir com amigos, família ou colegas de trabalho.
              </p>
            </div>
  ...
  // ── Slide 5: Recompensas (JBL) ─────────────────────────────────────────────
  const slideJBL = (
    <Slide id="recompensas">
      <div className="max-w-lg mx-auto w-full" data-aos="zoom-in">
        <div
  ...
  // ── Slide 6: CTA Final + Rodapé ────────────────────────────────────────────
  const slideCTA = (
    <div id="cta-final" className="w-full h-full flex flex-col">
      {/* Marquee strip */}
      <div className="overflow-hidden border-b border-border/30 bg-black/30 py-1 flex-shrink-0" aria-hidden="true">
      ...
      {/* CTA body */}
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-8 text-center" data-aos="fade-up">
      ...
      {/* Footer */}
      <footer className="flex-shrink-0 border-t border-border/40 bg-black/30 px-4 py-5 text-center">
  ...
  return (
    <>
      ...
      <SnapContainer
        ...
        onFirstScroll={() => setHintVisible(false)}
      />
    </>
  );
};
