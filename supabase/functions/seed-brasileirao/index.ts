/**
 * seed-brasileirao — insere jogos do Brasileirão para testes
 *
 * Segurança: requer autenticação de admin.
 * Sem isto, qualquer pessoa podia resetar toda a grade de jogos.
 */
...
serve(async (req) => {
  ...
  try {
    ...
    const { error } = await supabase.from("matches").insert(BRASILEIRAO_MATCHES);
